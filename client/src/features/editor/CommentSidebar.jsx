  ...
  return (
    <aside className="w-80 lg:w-96 flex-shrink-0 glass-panel rounded-3xl border-2 border-slate-900 dark:border-slate-700 shadow-2xl p-5 flex flex-col justify-between transition-colors animate-in fade-in slide-in-from-right-4 duration-200">
      <div className="flex-1 flex flex-col min-h-0">
        ...
        {/* Filter Tabs */}
        <div className="flex items-center gap-1 mt-3 p-1 bg-slate-200 dark:bg-black rounded-xl text-xs font-black">
          <button
            onClick={() => setFilter("all")}
            className={`flex-1 py-1.5 rounded-lg text-center transition-all ${
              filter === "all"
                ? "bg-black text-white dark:bg-white dark:text-black shadow"
                : "text-slate-900 dark:text-slate-200 hover:text-black"
            }`}
          >
            All ({comments.length})
          </button>
          ...
          {displayedComments.length > 0 ? (
            displayedComments.map((comment) => {
              ...
              return (
                <div
                  key={comment._id}
                  className={`p-3.5 rounded-2xl border-2 transition-all bg-slate-50 dark:bg-slate-950 ${
                    comment.isResolved
                      ? "border-emerald-600 bg-emerald-50 dark:bg-emerald-950/20"
                      : "border-slate-900 dark:border-slate-700 hover:border-indigo-600"
                  }`}
                >
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] font-mono font-bold text-slate-900 dark:text-slate-300">
                    ...
                    <div className="flex items-center gap-1">
                        ...
                        <button
                          ...
                          className={`p-1.5 rounded-lg font-bold transition-colors ${
                            comment.isResolved
                              ...
                              : "text-white bg-black hover:bg-emerald-600"
                          }`}
                      ...
                      {(comment.author?._id === currentUserId ||
                        ...
                        <button
                          ...
                          className="p-1.5 rounded-lg bg-black text-white hover:bg-rose-600 transition-colors"
                          title="Delete Thread"
                        >
                  ...
                  {editingCommentId === comment._id ? (
                    <div className="mt-2.5 space-y-2">
                      <textarea
                        ...
                        className="w-full bg-white dark:bg-black text-black dark:text-white border-2 border-black rounded-xl p-2.5 text-xs font-bold focus:outline-none focus:ring-2 focus:ring-indigo-600"
                        rows={2}
                      />
                      <div className="flex items-center justify-end gap-2">
                        <button
                          onClick={() => setEditingCommentId(null)}
                          className="px-2.5 py-1 text-xs font-black text-black dark:text-white hover:underline"
                        >
                          Cancel
                        </button>
                        <Button
                    ...
                    <p className="text-xs font-extrabold mt-2.5 leading-relaxed text-black dark:text-white">
                      {comment.content}
                    </p>
                  )}
                  ...
                  {/* Resolved Banner */}
                  {comment.isResolved && (
                    <div className="mt-2 text-[11px] font-black text-emerald-700 dark:text-emerald-400 flex items-center gap-1">
                  ...
                  {hasReplies && (
                    <button
                      ...
                      className="mt-2.5 text-[11px] font-black text-indigo-700 dark:text-indigo-400 hover:underline flex items-center gap-1 transition-colors"
                    >
                      {isExpanded ? (
                        <>
                  ...
                  {/* Replies Thread List (Expandable / Collapsible) */}
                  {hasReplies && isExpanded && (
                    <div className="mt-2 pt-2 border-t-2 border-slate-900 dark:border-slate-800 space-y-2 pl-2 animate-in fade-in duration-200">
                      {comment.replies.map((reply) => (
                        <div key={reply._id} className="flex items-start gap-2 text-xs group/reply">
                          <CornerDownRight className="w-3 h-3 text-black dark:text-white shrink-0 mt-1" />
                          <div className="flex-1 bg-white dark:bg-black p-2.5 rounded-xl border-2 border-black dark:border-slate-800">
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-1.5">
                                <Avatar
                                  ...
                                  size="xs"
                                />
                                <span className="font-black text-indigo-700 dark:text-indigo-400 text-[11px]">
                                  {reply.author?.name}
                                </span>
                              </div>

                              <div className="flex items-center gap-1">
                                <span className="text-[10px] font-mono font-bold text-black dark:text-slate-300">
                            ...
                            <p className="mt-1 text-[11px] font-black text-black dark:text-white">
                              {reply.content}
                            </p>
                          </div>
                        </div>
                  ...
                  {canComment && !comment.isResolved && (
                    <div className="mt-2.5 pt-2 border-t border-slate-900 dark:border-slate-800">
                      {replyingToId === comment._id ? (
                        <div className="space-y-2 mt-1">
                          <input
                            ...
                            className="w-full bg-white dark:bg-black text-black dark:text-white border-2 border-black rounded-xl px-2.5 py-1.5 text-xs font-bold focus:outline-none focus:ring-2 focus:ring-indigo-600"
                          />
                          <div className="flex items-center justify-end gap-2">
                            <button
                              type="button"
                              ...
                              className="text-xs font-black text-black dark:text-white hover:underline px-2 py-1"
                            >
                              Cancel
                            </button>
                            <Button
                        ...
                        <button
                          ...
                          className="text-[11px] font-black text-indigo-700 dark:text-indigo-400 hover:underline transition-colors"
                        >
                          + Reply
                        </button>
                      )}
            ...
            <p className="text-center text-xs font-black text-black dark:text-slate-200 py-10">
              {filter === "all"
        ...
        {/* New Comment Input */}
        {canComment && (
          <form onSubmit={handleAddComment} className="pt-3 border-t-2 border-black dark:border-slate-700 space-y-2">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-white dark:bg-black text-black dark:text-white border-2 border-black rounded-xl px-3 py-2 text-xs font-bold placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-600"
              />
