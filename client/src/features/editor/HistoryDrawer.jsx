...
export const HistoryDrawer = ({
  ...
  const displayedRevisions = showAllRevisions ? revisions : revisions.slice(0, 5);

  return (
    <>
      {/* Standard Google Docs Style Sidebar (Flat layout, no shadow, clean border-l) */}
      <aside className="w-80 lg:w-96 flex-shrink-0 bg-white dark:bg-slate-950 border-l border-slate-200 dark:border-slate-800 p-5 flex flex-col justify-between transition-colors animate-in fade-in slide-in-from-right-4 duration-200 text-slate-900 dark:text-white">
        <div className="flex-1 flex flex-col min-h-0">
            ...
            <form onSubmit={handleCreateRevision} className="mt-3 space-y-2">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-white dark:bg-slate-900 text-slate-900 dark:text-white border border-slate-300 dark:border-slate-700 rounded-xl px-3 py-1.5 text-xs font-bold focus:outline-none focus:ring-2 focus:ring-indigo-600"
                />
          ...
          {/* Revision List (Scrollable Flat Container) */}
          <div className="flex-1 overflow-y-auto mt-4 space-y-3 pr-1 max-h-[calc(100vh-230px)] scrollbar-thin">
            {loading ? (
              ...
              <>
                {displayedRevisions.map((rev) => {
                  ...
                  return (
                    <div
                      key={rev._id}
                      className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-indigo-500 transition-all flex flex-col gap-2.5"
                    >
                ...
                {revisions.length > 5 && (
                  <button
                    ...
                    className="w-full py-2.5 px-3 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-indigo-600 dark:text-indigo-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all text-xs font-extrabold flex items-center justify-center gap-1.5 mt-2"
                  >
                    {showAllRevisions ? (
                      <>
                        <ChevronUp className="w-4 h-4" />
                        <span>Show 5 Most Recent Only</span>
                      </>
                    ) : (
                      <>
                        <ChevronDown className="w-4 h-4" />
                        <span>Show More Revisions ({revisions.length - 5} older)</span>
                      </>
                    )}
                  </button>
                )}
